    ...
    ui(new Ui::NumismatistHandbook)
{
    ...
    currentFilenameLabel = new QLabel();
    ui->statusBar->addWidget(currentFilenameLabel);
...
void NumismatistHandbook::closeEvent(QCloseEvent *event)
{
    if (fileChanged())
    {
        auto result = askToSafe(/*includeCancelButton = */true);

        if (result == SafeAccepted)
            saveFileSlot();

        if (result == EventCanceled)
            event->ignore();
        else
            event->accept();
    }
...
void NumismatistHandbook::showCurrentFilename()
{
     ...
     currentFilenameLabel->setText(message);
}
...
void NumismatistHandbook::createItemSlot()
{
    if (model.count() < MAX_ITEMS_COUNT)
    {
        ...
        int newIndex = model.append(newItem);
